...
def Train(X, y, bandwidth, transfer_path, base_path, save_path, dropout_p, width, transfer, learning_rate, epochs=1000, device='cpu'):
    ...
    Var_bw=Loss_function.Variable_bandwidth(x=epsilon,bandwidth=bandwidth, device=device)
    ...
    for epoch in tqdm(range(epochs), desc='train EML', position=1, leave=False):
        EML_FNN.train()
        running_losskde = 0.0
    ...
    return save_path, intercept, EML_FNN

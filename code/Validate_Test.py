...
def Test(X, y, intercept, save_path, dropout_p, width, device='cpu'):
    input_dim = X.shape[1]
    EML_FNN = Network.FNN(input_dim=input_dim, init_weights=True,dropout_p=dropout_p,width=width).to(device)
